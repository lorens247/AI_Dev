-- Fix Database Schema for Poll Creation
-- Run this in your Supabase SQL Editor to fix the foreign key constraint issue

-- First, drop the existing tables in the correct order
DROP TABLE IF EXISTS public.votes CASCADE;
DROP TABLE IF EXISTS public.poll_options CASCADE;
DROP TABLE IF EXISTS public.polls CASCADE;

-- Recreate the polls table with correct foreign key reference
CREATE TABLE public.polls (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  title TEXT NOT NULL,
  description TEXT,
  created_by UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL, -- Changed from profiles to auth.users
...
-- Recreate the poll options table
...
-- Recreate the votes table
CREATE TABLE public.votes (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  poll_id UUID REFERENCES public.polls(id) ON DELETE CASCADE NOT NULL,
  option_id UUID REFERENCES public.poll_options(id) ON DELETE CASCADE NOT NULL,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL, -- Changed from profiles to auth.users
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
-- Verify the fix
SELECT 'Schema fix completed. Polls table structure:' as info;
